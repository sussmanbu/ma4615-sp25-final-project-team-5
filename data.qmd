...
df_clean <- df_clean |> group_by(YEAR, `RACE/ETHNICITY`, SEX) |> mutate(count=n()) |> ungroup() # this is supposed to see how many of each sex are within every race/year 
...
df_clean |> filter(`Neighborhood (U.H.F)`=="All" & Borough=="All")|> ggplot(
  mapping = aes(x = YEAR, color = SEX , y = count)) + geom_point() + geom_line() + facet_wrap(~ `RACE/ETHNICITY`) # count of each sex faceted by race 
...
df_clean |> filter(`Neighborhood (U.H.F)`=="All" & Borough=="All")
