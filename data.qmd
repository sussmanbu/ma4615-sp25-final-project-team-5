...
```{r}
df_clean |> filter(`Neighborhood (U.H.F)`=="All" & SEX=="All"& Borough=="All")

df_clean |> filter(`Neighborhood (U.H.F)`=="All" & SEX=="All"& Borough=="All") |> ggplot(
...
df_clean |> filter(`Neighborhood (U.H.F)`=="All" & SEX!="All"& `RACE/ETHNICITY`!="All") |> ggplot(
  mapping = aes(x = YEAR, y = `PROPORTION OF CONCURRENT HIV/AIDS DIAGNOSES AMONG ALL HIV DIAGNOSES`)) + geom_point() + facet_wrap(~ `Borough`)

df_clean |> ggplot(
  mapping = aes(x = YEAR, y = `PROPORTION OF CONCURRENT HIV/AIDS DIAGNOSES AMONG ALL HIV DIAGNOSES`)) + geom_point() + facet_wrap(Borough~`SEX`) 
